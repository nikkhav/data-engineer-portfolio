...
class CurrenciesLoader:
    def __init__(
        ...
        self.workflow_key = "currencies_load"
        self.target_table = "VT25112277C9AC__STAGING.currencies"
        self.reject_table = "VT25112277C9AC__STAGING.currencies_rejects"

    def load(self):
        self.log.info("Loading currencies...")

        with self.pg_conn.connection() as pg:
            df, last_date = self.s3_repo.list_currencies(
                pg_conn=pg,
        ...
        if df.empty:
            self.log.info("No new currencies to load.")
            return 0
        ...
        with self.pg_conn.connection() as pg:
            self.wf_repo.save_setting(
                pg,
                self.workflow_key,
                {"last_loaded_date": last_date.isoformat()}
            )
        self.log.info(f"Workflow state updated: last_loaded_date = {last_date.isoformat()}")
